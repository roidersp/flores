...
if( !function_exists('ftc_blogs_shortcode') ){
	function ftc_blogs_shortcode( $atts, $content = null){
		...
		if( $posts->have_posts() ):
			...
			$classes = array();
			$classes[] = 'ftc-sb-blogs ftc-shortcode';
			if( $is_slider ){
	...
	if( !function_exists('ftc_products_slider_shortcode') ){
		function ftc_products_slider_shortcode($atts, $content){
			...
			if( $products->have_posts() ): 
				?>
			<div class="<?php echo esc_attr(implode(' ', $classes)); ?>" data-slider="<?php echo esc_attr((int)$is_slider) ?>" data-columns="<?php echo esc_attr($columns) ?>" data-auto_play="1"
